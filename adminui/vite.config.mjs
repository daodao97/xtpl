...
const buildOutDir = 'ui'

const injectBaseUrlScript = () => ({
      ...
      tags: [
        {
          ...
          children: "window.BASE_URL = '/' + location.pathname.split('/').filter(e => e).concat(['api']).join('')"
        }
      ]
    }
  }
...
export default ({ mode }) => {
  ...
  return defineConfig({
    ...
    build: {
            ...
            targets: [
              {
                ...
                dest: `${buildOutDir}/assets/monaco-editor/vs`
              }
            ],
            hook: 'writeBundle'
          })
      ...
      chunkSizeWarningLimit: 1000, // 调整块大小限制
      outDir: buildOutDir
    },
    plugins
  })
